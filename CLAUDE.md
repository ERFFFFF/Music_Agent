...
- **Session first, password only as a fallback**: the signed cookie round-trips through the config (`CadenceClient(session=…, on_session=…)`, wired by `backends.cadence.client_from_config`) and is what every request actually uses. The account is stored too now (GUI only — see `cadence_username` below), but only to re-sign in when that cookie is gone. **Starlette only re-issues the cookie when the session is MODIFIED** (verified in 1.3.1's source; an authenticated GET returns no Set-Cookie) — so Cadence's `_touch_session` marks it modified on `/api/remote/command|state`, which is what makes the agent's saved session slide instead of dying 7 days after login. Not done on `/api/remote/pending` (the browser's 1/s poll).
...
- **`cadence_username` / `cadence_password` ARE in `DEFAULTS` now** (they were not, and the reason they were not is worth knowing: `save_config` is built from `DEFAULTS`, so a field outside it cannot reach disk at all). The GUI stores them because its sign-in window shows you what it already has and re-signs in without a window when a session ages out — an app that asks for a password it refuses to remember is just asking twice. They are in `SECRET_FIELDS`, so they are DPAPI-sealed like everything else; `login.LoginWindow._submit` writes them only AFTER the login succeeds, so a typo is never stored and retried; and Settings' Sign out clears them along with the cookie or the next launch would undo the sign-out. The CLI is unaffected: a `.env`-supplied account still hits the "what the .env supplies, the .env keeps" rule and is written back as `""`.
- **`proxy_user` and `proxy_password` are sealed too**, and the proxy is reachable from the sign-in window (`login.proxy_dialog`), not just Settings — on a network that mandates a proxy, the sign-in window is the first thing to fail and the tray icon that leads to Settings does not exist yet. `tests/test_integration.py::test_a_proxy_from_the_config_file_actually_carries_the_request` runs a real proxy on loopback and asks for a `.invalid` host, so the request can only arrive if the proxy carried it; it also decodes the `Proxy-Authorization` header to pin `config.proxy_url()`'s percent-encoding against urllib's `unquote`.
- **Every masked field uses `ui/widgets.secret_entry`** — one entry + eye toggle, in its own module because `ui/settings.py` keeps `ui/login.py` out of its import-time graph. The glyphs are BMP (`◉` U+25C9 / `⌽` U+233D): Tcl 8.6 cannot hold a character above U+FFFF, so the obvious 👁 raises the moment the button is built.
